    ...
    'label: loop {
        ...
        match op_code {
            ...
            OpCode::FfMStoreFromSignal => {
                ...
                let src_signals_start = component_tree.signals_start;
                let src_start = src_signals_start
                    .checked_add(sig_idx)
                    .ok_or(RuntimeError::SignalIndexOutOfBounds)?;
                ...
                for offset in 0..size {
                    ...
                    let value = signals[sidx].ok_or(RuntimeError::SignalIsNotSet)?;

                    let didx = dst_start
                        .checked_add(offset)
                        .ok_or(RuntimeError::MemoryAddressOutOfBounds)?;
                    if didx >= vm.memory_ff.len() {
                        vm.memory_ff.resize(didx + 1, None);
                    }
                    vm.memory_ff[didx] = Some(value);
                }
                ...
                #[cfg(feature = "debug_vm2")]
                {
                    println!(
                        ...
                        size, src_start, src_start + size, dst_start, dst_start + size
                    );
                }
            }
            OpCode::FfMStoreFromCmpSignal => {
            ...
            OpCode::CopyCmpInputsFromMemory => {
                ...
                let component = match component_tree.components.get_mut(dst_cmp_idx) {
                    Some(Some(c)) => c,
                    _ => return Err(Box::new(RuntimeError::UninitializedComponent)),
                };
                ...
                for offset in 0..num_signals {
                    ...
                    let dst_idx_global = dst_signals_start
                        .checked_add(dst_sig_idx)
                    ...
                    let dst_slot = signals
                        .get_mut(dst_idx_global)
                        .ok_or(RuntimeError::SignalIndexOutOfBounds)?;
                    if dst_slot.is_some() {
                        return Err(Box::new(RuntimeError::SignalIsAlreadySet));
                    }
